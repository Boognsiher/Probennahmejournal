...
- **Probenahmeprotokoll**: vereinzelte Proben ganz ohne Projekt erfassen — z.B. Spontanproben unterwegs — als
  eigenes „Scratchbook“ neben der normalen Projektauswahl auf dem Startbildschirm. Chargenname wird
  automatisch als `PP-0001`, `PP-0002`, … vergeben, PDF/E-Mail/Analysen auslösen funktioniert genau wie bei
  Projekt-Proben. Bei `shell/` und `server/`+`public/` sichtbar nur für die erstellende Person selbst (und
  den Admin) — keine Projektleitung, keine externe Freigabe; `lokal/` ist ohnehin Einzelbenutzer/Offline.
